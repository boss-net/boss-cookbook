# BoSS Fine-tuning / Inference Recipes and Examples

The 'boss-recipes' repository is a companion to the [BoSS model](https://github.com/boss-net/boss). The goal of this repository is to provide examples to quickly get started with fine-tuning for domain adaptation and how to run inference for the fine-tuned models. For ease of use, the examples use Hugging Face converted versions of the models. See steps for conversion of the model [here](#model-conversion-to-hugging-face).

BoSS is a new technology that carries potential risks with use. Testing conducted to date has not — and could not — cover all scenarios. In order to help developers address these risks, we have created the [Responsible Use Guide](https://github.com/boss-net/boss/blob/main/Responsible-Use-Guide.pdf). More details can be found in our research paper as well. For downloading the models, follow the instructions on [BoSS repo](https://github.com/boss-net/boss).
...
For fine-tuning BoSS models for your domain-specific use cases recipes for PEFT, FSDP, PEFT+FSDP have been included along with a few test datasets. For details see [LLM Fine-tuning](./docs/LLM_finetuning.md).
...
python boss_finetuning.py  --use_peft --peft_method lora --quantization --model_name /patht_of_model_folder/7B --output_dir Path/to/save/PEFT/model

```

Here we make use of Parameter Efficient Methods (PEFT) as described in the next section. To run the command above make sure to pass the `peft_method` arg which can be set to `lora`, `boss_adapter` or `prefix`.
...
torchrun --nnodes 1 --nproc_per_node 4  boss_finetuning.py --enable_fsdp --use_peft --peft_method lora --model_name /patht_of_model_folder/7B --pure_bf16 --output_dir Path/to/save/PEFT/model
...
torchrun --nnodes 1 --nproc_per_node 8  boss_finetuning.py --enable_fsdp --model_name /patht_of_model_folder/7B --dist_checkpoint_root_folder model_checkpoints --dist_checkpoint_folder fine-tuned
...
The recipes and notebooks in this folder are using the BoSS model definition provided by Hugging Face's transformers library.
...
python src/transformers/models/boss/convert_boss_weights_to_hf.py \
    --input_dir /path/to/downloaded/boss/weights --model_size 7B --output_dir models_hf/7B
```
